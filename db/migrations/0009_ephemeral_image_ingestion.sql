ALTER TABLE reports
  ...
  ADD COLUMN IF NOT EXISTS ocr_language_set text;

CREATE OR REPLACE FUNCTION accept_image_investigation(
  p_short_id text,
  p_report_locale varchar(2),
  p_visitor_key text,
  p_network_key text,
  p_idempotency_key text,
  p_analyzed_excerpt text,
  p_extracted_word_count integer,
  p_analyzed_word_count integer,
  p_truncated boolean,
...
BEGIN
  ...
  SET source_kind = 'image',
      source_url = NULL,
      analyzed_excerpt = p_analyzed_excerpt,
      extracted_word_count = p_extracted_word_count,
      analyzed_word_count = p_analyzed_word_count,
      truncated = p_truncated,
      ocr_confidence = p_ocr_confidence,
      ocr_language_set = 'eng+spa+fra+por',
      updated_at = now()
  WHERE id = (v_decision->'report'->>'id')::uuid
